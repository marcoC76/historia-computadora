...
var Timeline = function () {
    ...
    return _createClass(i, [{
        key: "init",
        value: function () {
            if (this.data && (this.data = this.data.sort(this.sortNodesByDate("year")), this.injectHTML(), this.timelineInner = document.querySelector("#".concat(this.wrapperId, " .timeline")), this.nodes = document.querySelectorAll("#".concat(this.wrapperId, " .tl-node")), this.checkScreenSize())) {
        ...
        value: function () {
            var e = this.getBoundingClientRect().left,
                t = this.children[2];
            e > window.innerWidth / 2 ? (t.style.left = "auto", t.style.right = "-20px", t.classList.remove("left-triangle"), t.classList.add("right-triangle")) : (t.style.left = "-20px", t.style.right = "auto", t.classList.add("left-triangle"), t.classList.remove("right-triangle"))
        }
    }, {
        ...
        value: function () {
            ...
            for (var s = 0; s < this.data.length; s++) {
                ...
                .concat(this.data[s].title, `</h3><br> <h5>${this.data[s].text}</h5>\n          </div>\n          <img src="`)
